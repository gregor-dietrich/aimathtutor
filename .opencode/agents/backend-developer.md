...
- **AI config**: Runtime-mutable via `AiConfigService` (DB-backed). API keys from properties (`app.google.api.key`, `app.openai.api.key`, `app.openai.organization-id`).
...
6. Run `make lint` to lint, and `make test` to validate
